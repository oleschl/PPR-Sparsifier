...
int update(MinDegreePQ &pq, int tag, int min_deg, std::vector<int> & xadj, std::vector<int> &adj, int v, std::vector<int> &super_size, std::vector<int> &marker, std::vector<int> &needs_update, std::vector<int> &tmp_next, std::vector<int> &inv_perm, std::vector<int> &merge_parent, std::vector<int> &map_G_to_invK){
    ...
    std::cout << "min deg: " << min_deg << " num neighbors: " << count << std::endl;
...
std::pair<std::vector<int>, std::vector<int>> minimum_degree_ordering(int n, int m, std::vector<int> &xadj, std::vector<int> &adj, std::vector<int>& K, std::vector<int> & inv_K){
    ...
    while(num <= inv_K.size()){
        ...
        if(num + super_size[eliminationVertex] > inv_K.size()){
            break;
        }
        ++tag;
